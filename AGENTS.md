...
- `profiles/*.list` 声明按项目形态投放的 Skill 组合，是本仓库与安装器之间唯一的耦合缝；组合的归属方是本仓库，安装器不得硬编码任何 Skill 名称。
- 新增、重命名或删除 Skill 时同步更新 `profiles/*.list`，并确认每个 Skill 至少被一个清单覆盖。
- 清单为纯文本：一行一个 Skill 目录名，`#` 开头为注释，文件首行注释即该组合的展示说明。
- 每个清单自包含并显式列全，不使用引用或继承；组合之间的重复条目是刻意保留的，以换取零解析逻辑和可直接通读的清单。
