...
7. 数据结构和业务错误字段使用 `$dayu-sec-data-contract-first`，API 设计使用 `$dayu-sec-rest-api`，包导出与类型解析使用 `$dayu-sec-typescript`。
8. 任务触及微前端宿主、子应用生命周期、装配、隔离或跨应用契约时，同时使用 `$dayu-sec-web-micro-frontend`；本 Skill 只定义应用内部请求层。
