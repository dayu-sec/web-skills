...
5. 涉及响应字段、错误体或已有接口结构时，同时使用 `$dy-sec-data-contract-first`；涉及 URL、HTTP 方法、状态码、分页或 REST 错误语义时同时使用 `$dy-sec-rest-api`；涉及 TypeScript 类型和模块解析时同时使用 `$dy-sec-typescript`。
6. 涉及 SDK 接入、手写业务 Service、请求实例、Base URL、拦截器或错误传播时同时使用 `$dy-sec-frontend-request`。只有任务触及宿主、子应用、生命周期、隔离或跨应用 Mock 契约时才额外使用 `$dy-sec-microfrontend`。
