...
- 品牌色、密度、排版和页面骨架使用目标项目已确认的规范或对应产品 UI/UX Skill；内部产品和管理后台使用 `$dayu-sec-ui-product-internal`，面向客户和终端用户的产品使用 `$dayu-sec-ui-product-client`。React 组件职责与 Hooks 使用 `$dayu-sec-react`；微前端生命周期与跨应用契约使用 `$dayu-sec-web-micro-frontend`。
