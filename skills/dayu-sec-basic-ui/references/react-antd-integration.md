# DayuSec React、Ant Design 与主题接入
...
- 独立运行与集成运行复用同一主题装配契约，并按 `$dayu-sec-web-micro-frontend` 验证挂载、卸载、主题同步和 Portal 清理。
...
- 组件 API、状态或 Semantic DOM 不确定时，使用 `$dayu-sec-antd-ui-components` 查阅组件级资料。
...
- 微前端宿主与子应用之间的主题同步、选择器隔离和卸载验证由 `$dayu-sec-web-micro-frontend` 约束。
