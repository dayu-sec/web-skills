...
7. 按实际代码组合任务 Skill：React 使用 `$dayu-sec-react`，shadcn/ui 与 Tailwind CSS 使用 `$dayu-sec-ui-shadcn`，产品视觉与主题使用目标项目已确认的规范或对应产品 UI/UX Skill，数据请求使用 `$dayu-sec-web-request`，TypeScript 使用 `$dayu-sec-typescript`。
