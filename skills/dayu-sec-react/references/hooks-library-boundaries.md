...
服务端状态只在这里声明归属；请求实例、业务 Service、错误传播和 Query 状态职责使用 `$dayu-sec-web-request`，不要在本文件复制实现规则。
...
- 只有任务明确触及共享依赖装配、运行时单例、生命周期或隔离时才组合 `$dayu-sec-web-micro-frontend`；普通应用内部 Hook 不因仓库采用微前端而改变实现。
